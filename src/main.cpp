...
int64 nChainStartTime = 1453482000;
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // FuturePoints: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
     ...
     if(nHeight == 1)
     {
       nSubsidy = 360000000 * COIN;	// 36 million coins, that all pow coins
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "FuturePoints";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1453482000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1650768;
        ...
        assert(block.hashMerkleRoot == uint256("0x906159f0cdee65e4726c729b0a8726bc387ddecaf1120a07892ff6f70994761e"));
...
unsigned char pchMessageStart[4] = { 0xce, 0xaf, 0xca, 0xdd };
